                        ...
                        @if((isset($data->id) && 1 <> $data->id) || !isset($data->id))
                            <div class="col-sm-4 col-12">
                                <div class="mb-10 fv-row fv-plugins-icon-container">
                                    ...
                                    <select name="parentName" class="form-select form-select-solid form-select-lg fw-semibold select2">
                                        <option value="">Select</option>
                                        @foreach ($collection as $item)
                                            <option value="{{ $item->id }}" {{ (((isset($data['parentId']) && $data['parentId'] == $item->id) || old('parentName') == $item->id) ? 'selected' : '')  }}>{{ $item->nameEn }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                @error('parentName')
                    ...
                    <div class="row">
                        <div class="col-sm-4 col-12">
                            <div class="mb-10 fv-row fv-plugins-icon-container">
                                ...
                                <input type="file" name="image" accept="image/jpg, image/svg, image/webp, image/png, image/jpeg" class="form-control form-control-lg form-control-solid rounded" placeholder="Image" value="{{ $data['image'] ?? old('image') }}">
                                @error('image')
                            ...
                            @isset($data->image)
                                @if(file_exists(public_path('storage/uploads/picture/'.$data->image)))
                                    <a href="#" title="{{ $data->nameEn }}" target="_blank">
                                        <img style="width:50px;height:50px;" class="rounded-circle bg-primary" src="{{ asset('storage/uploads/picture/'.$data->image) }}" alt="{{ $data->image }}">
                                    </a>
                                @elseif($data->id == 1)
                                    <a href="#" title="{{ $data->nameEn }}" target="_blank">
                                        <img style="width:50px;height:50px;" class="rounded-circle bg-primary" src="{{ asset('assets/imgs/ceo.png') }}" alt="{{ $data->image }}">
                                    </a>
                                @endif
                            @endisset
                        </div>
