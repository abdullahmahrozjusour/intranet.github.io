...
<body>
    ...
    <div class="header-home bg-[#21295B]">
        ...
        'policies' => $modalData['Information Technology'][0],
        ...
        <div class="fixed left-0 right-0 top-0 bottom-0 h-full w-full bg-black/50 overflow-x-hidden overflow-y-auto hidden"
            ...
            <div class="max-w-[550px] w-full mx-auto rounded-lg overflow-hidden relative ">
                <div
                    class="bg-secondary relative text-center font-semibold text-2xl flex justify-center p-2 text-white">
                    Policies & Procedures
                ...
                <div class="bg-modal">
                    <div class="px-8 py-4 content-modal">
                        ...
                        @foreach ($modalData as $key => $item)
                        <div class="mb-1 mt-5 text-xl font-semibold">{{ $key }}</div>
                        <div class="flex gap-4">
                            @foreach ($item as $pp)
                            <a @if($pp['titleEn']=='Policies' ) href="{{ route('policy',[$key]) }}" class="btn-blue"
                                @elseif($pp['titleEn']=='Procedures' ) href="{{ route('procedure',[$key]) }}"
                                class="btn-white" @endif>{{ $pp['titleEn']
                                }}</a>
                            @endforeach
                        </div>
                        @endforeach
